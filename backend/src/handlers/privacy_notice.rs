...
pub async fn show(tmpl: web::Data<tera::Tera>) -> Result<HttpResponse, Error> {
    ...
    let rendered = tmpl.render("privacy_notice.html", &ctx).unwrap();
    Ok(HttpResponse::Ok().content_type("text/html").body(rendered))
}

#[cfg(test)]
mod tests {
    use crate::handlers::privacy_notice::show;
    ...
    async fn unit_privacy_page() {
        ...
        assert!(body.as_str().contains("Datenschutz"))
    }
}
