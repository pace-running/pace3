...
pub fn send_payment_confirmation(email: String, donation: String, status_link: String) -> bool {
    dotenv().ok();
    let sender_email =
        std::env::var("SENDER_EMAIL").unwrap_or_else(|_| "SENDER_EMAIL must be set.".to_string());
    let smtp_password =
    ...
    let tmpl = web::Data::new(tera);
    ctx.insert("email_info", &email_info);
    let rendered = tmpl.render("payment_confirmation_mail.html", &ctx).unwrap();
    if sender_email.contains("must be set") || smtp_password.contains("must be set") {
        return false;
    }
    let email = Message::builder()
