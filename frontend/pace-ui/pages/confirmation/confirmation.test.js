...
describe('should test confirmation page', () => {
    ...
    test('static elements are rendered', () => {
        ...
        expect(screen.getByText('Super! Du hast dich vorläufig für den Lauf angemeldet. Um die Registrierung abzuschließen, überweise bitte deinen Spendenbetrag an folgendes Konto:'));
    ...
    test('dynamic elements are rendered', () => {
        ...
        expect(screen.getByRole('link', {name: 'Meinen Anmeldestatus abrufen'})).toHaveAttribute('href','/status/?runner_id=123&start_number=125&verification_code=verificationCode');
        expect(screen.queryByText('Wir haben dir zudem diese Bestätigung an deine hinterlegte E-Mail Adresse gesendet.')).not.toBeInTheDocument();
    });
